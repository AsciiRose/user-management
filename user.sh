...
if [ $# -lt 3 ]
	...
	if [ $(cat /etc/passwd | grep $1 | wc -w) -gt 0 ]										#Prüfen ob Nutzer schon besteht
		then
			echo "Nutzer besteht bereits"													#Fehlermeldung ausgeben das der Nutzer bereits existiert
			exit 2
		else
			echo "$1:x:501:501:$2:/home/$1:/bin/bash" >> /etc/passwd						#Nutzer ini passwd Datei anlegen -> Name:Passwort:User-ID:Group-ID:Kommentar:Verzeichnis:Shell  
